...
pub async fn syllable_router(request: &str) -> Option<(String, String)> {
    match request {
        ...
        r if r.starts_with("GET /api/syllable/") => Some(handle_get_syllables_request(r).await),
        r if r.starts_with("GET /api/syllable") => Some(handle_get_all_syllables_request(r).await),
        r if r.starts_with("PUT /api/syllable/") => Some(handle_put_syllables_request(r).await),
