...
pub async fn user_router(request: &str) -> Option<(String, String)> {
    match request {
        ...
        r if r.starts_with("GET /api/user/") => Some(handle_get_user_request(r).await),
        r if r.starts_with("GET /api/user") => Some(handle_get_all_users_request(r).await),
        r if r.starts_with("PUT /api/user/") => Some(handle_put_user_request(r).await),
        r if r.starts_with("DELETE /api/user/") => Some(handle_delete_user_request(r).await),
        _ => None,
    }
}
