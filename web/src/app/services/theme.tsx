...
export function getJapanese(syllabaryRecord: SyllabaryRecord, matchText: [string[], string[]], inputText: string, matchLength: number, force: boolean) : [[string[], string[]], string] {
    ...
    if (matchLength === 3) {
        if (processText.length >= matchLength) {
            const [rest, translation] = tryParse(syllabaryRecord, processText, 3, force);
        ...
        if (processText.length >= matchLength) {
            const [rest, translation] = tryParse(syllabaryRecord, processText, 2, force);
            const [hiraganaSyllabary, katakanaSyllabary] = translation;
            if (hiraganaSyllabary && katakanaSyllabary) {
                matchText[0].push(hiraganaSyllabary);
                matchText[1].push(katakanaSyllabary);
        ...
        if (processText.length >= matchLength) {
            const [rest, translation] = tryParse(syllabaryRecord, processText, 1, force);
            const [hiraganaSyllabary, katakanaSyllabary] = translation;
            if (hiraganaSyllabary !== "" && katakanaSyllabary !== "") {
                matchText[0].push(hiraganaSyllabary);
                matchText[1].push(katakanaSyllabary);
...
function tryParse(syllabaryRecord: SyllabaryRecord, inputText: string, length: number, force: boolean): [string, [string, string]] {
...
export function getRoman(syllabaryRecord: SyllabaryRecord, hiraganaArray: string[]) {
